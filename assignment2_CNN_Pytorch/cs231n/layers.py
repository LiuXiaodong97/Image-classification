...
def affine_backward(dout, cache):
    ...
    dw = np.reshape(x,(x.shape[0],-1)).T.dot(dout)  #(D,N)*(N,M)
...
def softmax_loss(x, y):
    ...
    correct_logprobs = -np.log(probs[range(num_input),y])
    loss = np.sum(correct_logprobs)/num_input
    
    # compute the gradient on scores
    dx = probs
    dx[range(num_input),y] -= 1
    dx /= num_input
...
def batchnorm_backward(dout, cache):
    ...
    dx = dout_hat/(std+eps)
    dmean = -dout_hat/(std+eps)
    dmean = np.sum(dmean,axis=0)  #(D 
    dstd = dout_hat * -(x-mean) / (std+eps)**2
    dstd = np.sum(dstd,axis=0)  #(D
    var = std**2
    dvar = dstd * 0.5 * var ** -0.5
    dvar /= N
    dmean += dvar * 2 * np.sum((mean-x),axis=0)
    dx += dvar * 2 * (x-mean)
    dx += dmean/N    #(N,D
...
def batchnorm_backward_alt(dout, cache):
    ...
    dmean = -dout_hat/(std+eps) + dvar * -2 * (x-mean)/N
    dmean = np.sum(dmean,axis=0)
    dx = dout_hat / (std+eps) + dvar*2*(x-mean)/N + dmean/N
...
def layernorm_backward(dout, cache):
    ...
    D = x_hat.shape[1] #x_hat (N,D
    dgamma = np.sum(dout*x_hat,axis=0)
    dbeta = np.sum(dout,axis=0)
    dx_hat = (dout*gamma).T
    x_hat = x_hat.T
    dx = 1./ D*inv_sigma * (D * dx_hat - np.sum(dx_hat,axis=0) - x_hat * np.sum(dx_hat*x_hat,axis=0))
    dx = dx.T
        ...
        for j in range(f):
            for _w in range(out_w):
                for _h in range(out_h):
                    out[i,j,_h,_w] = np.sum(x_pad[i,:,_h*stride[1]:_h*stride[1]+hh,_w*stride[0]:_w*stride[0]+ww]*w[j])+b[j]
        ...
        for j in range(f):
            for _w in range(out_w):
                for _h in range(out_h):
                    ...
                    db[j] += dout[i,j,_h,_w]
                    dx_pad[i,:,_h*stride[1]:_h*stride[1]+hh,_w*stride[0]:_w*stride[0]+ww] += dout[i,j,_h,_w]*w[j]
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(n):
        for j in range(c):
            for _h in range(out_h):
                for _w in range(out_w):
                    indices = np.unravel_index(np.argmax(x[i,j,_h*stride:_h*stride+pool_h,_w*stride:_w*stride+pool_w]),dims=(pool_h,pool_w))
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    N,C,H,W = x.shape
    x_new = np.transpose(x,axes=(0,2,3,1)).reshape((-1,C))
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    out = gamma*x_hat+beta
    inv_sigma = 1/np.sqrt(sigma2+eps)
    cache = (x_hat,gamma,mu,inv_sigma,G)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    x_hat,gamma,mu,inv_sigma,G = cache
    N,C,H,W = x_hat.shape
    dgamma = np.sum(dout*x_hat,axis=(0,2,3),keepdims=True)
    dbeta = np.sum(dout,axis=(0,2,3),keepdims=True)
    dx_hat = (dout*gamma).reshape((N*G,C//G*H*W)).T
